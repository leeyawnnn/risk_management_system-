...
double tail_mean(std::vector<double>& data, double tail_prob) {
  ...
  std::size_t k = static_cast<std::size_t>(std::floor(tail_prob * static_cast<double>(n)));
  if (k < 1) k = 1;
  if (k > n) k = n;

  // Partition so the k smallest values occupy positions [0, k).
  std::nth_element(data.begin(), data.begin() + static_cast<std::ptrdiff_t>(k - 1),
                   data.end());
  double sum = 0.0;
...
double historical_cvar(const Eigen::VectorXd& portfolio_returns,
  ...
  if (!(confidence > 0.0 && confidence < 1.0)) {
    throw std::invalid_argument("historical_cvar: confidence must be in (0, 1)");
  }
  if (horizon_days < 1) {
...
double parametric_cvar(double mean, double stdev, double confidence,
  ...
  if (!(confidence > 0.0 && confidence < 1.0)) {
    throw std::invalid_argument("parametric_cvar: confidence must be in (0, 1)");
  }
  if (horizon_days < 1) {
...
double monte_carlo_cvar(const Eigen::VectorXd& mean, const Eigen::MatrixXd& cov,
  ...
  if (!(confidence > 0.0 && confidence < 1.0)) {
    throw std::invalid_argument("monte_carlo_cvar: confidence must be in (0, 1)");
  }
