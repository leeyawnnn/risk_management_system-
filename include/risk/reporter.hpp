...
RiskReport build_report(
    const std::string& name, const Portfolio& portfolio,
    const Eigen::MatrixXd& asset_returns, const Eigen::MatrixXd& cov,
    const std::string& cov_method, double annualization_factor,
    const std::vector<double>& confidences, const std::vector<int>& horizons,
    const std::vector<StressResult>& stress,
    const std::map<std::string, std::string>& sectors = {},
    int mc_draws = 100000, std::uint64_t seed = 42, double ewma_lambda = 0.94);
